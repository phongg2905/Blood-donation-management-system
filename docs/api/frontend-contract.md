...
`CurrentUser = { id, email, fullName, phone?, address?, roles: RoleCode[], permissions: PermissionCode[] }`.

`phone`/`address` chỉ áp dụng cho tài khoản DONOR (lưu ở `DonorProfile`; trả `null` khi chưa có) và được trả lại trong `CurrentUser` từ `login`, `register`, `GET/PATCH /auth/me`; tài khoản STAFF/ADMIN không có trường này. Gửi 2 field này cho tài khoản STAFF/ADMIN sẽ trả `VALIDATION_ERROR`.

### Rate limiting (auth public endpoints)

`POST /auth/login`, `POST /auth/register` và `POST /auth/forgot-password`, `POST /auth/reset-password` bị giới hạn theo IP (mặc định 10 request/15 phút cho cặp đầu, 5 request/15 phút cho cặp sau; đặt lại qua `RATE_LIMIT_LOGIN_MAX`/`RATE_LIMIT_RESET_MAX`). Vượt hạn trả **HTTP 429** với envelope chuẩn và `error.code = "FORBIDDEN"` cùng thông báo tiếng Việt; FE chỉ cần hiện `error.message`. Response kèm `RateLimit` standard headers (draft-7).

### ⚠️ Chưa được PM chốt — cần xác nhận trước khi FE tích hợp

Hai endpoint dưới đây được implement theo checklist bàn giao BE→FE nhưng **chưa nằm trong phạm vi Phase 2 đã duyệt ở bảng trên**. Hiện không có endpoint tạo user nào khác (không có `POST /users`), nên nếu bỏ `register`, hệ thống hiện tại không có cách nào tạo tài khoản DONOR ngoài thao tác trực tiếp trên DB.

| Method | Path                    | Request DTO                             | Response DTO                                          | perm   | Error codes                                    |
| ------ | ----------------------- | --------------------------------------- | ----------------------------------------------------- | ------ | ---------------------------------------------- |
| POST   | `/auth/register`        | `{ email, password, fullName, phone? }` | `{ accessToken, user: CurrentUser }` + refresh cookie | public | `VALIDATION_ERROR`, `AUTH_EMAIL_EXISTS`        |
| POST   | `/auth/forgot-password` | `{ email }`                             | `{ message }` (+ `devResetToken` ngoài production)    | public | `VALIDATION_ERROR`                             |
| POST   | `/auth/reset-password`  | `{ token, newPassword }`                | `{}`                                                  | public | `VALIDATION_ERROR`, `AUTH_RESET_TOKEN_INVALID` |

`register` luôn tạo role `DONOR` — STAFF/ADMIN account chỉ do ADMIN tạo (chưa có endpoint, dự kiến Phase 8). `forgot-password` luôn trả cùng một response bất kể email có tồn tại hay không, để không lộ thông tin tài khoản.
...
| HTTP | Typical codes                                                                                                                                                                                                                                                                                                                                                                              |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 400  | `VALIDATION_ERROR`, `REQUEST_INVALID`, `CAMPAIGN_DATE_INVALID`, `CAMPAIGN_TARGET_INVALID`, `MEASUREMENT_INVALID`, `SCREENING_REVIEW_REASON_REQUIRED`, `SCREENING_TEST_CODE_INVALID`, `DONATION_VOLUME_INVALID`, `DONATION_RECONCILIATION_FAILED`, `PAYLOAD_TOO_LARGE`, `AUTH_RESET_TOKEN_INVALID`                                                                                          |
| 401  | `UNAUTHENTICATED`, `AUTH_INVALID_CREDENTIALS`, `AUTH_ACCOUNT_INACTIVE`, `AUTH_SESSION_INVALID`                                                                                                                                                                                                                                                                                             || 403  | `FORBIDDEN`                                                             |
| 429  | `FORBIDDEN` (rate limit — auth public endpoints, kèm thông báo chờ)                                                                                       |
| 404  | `ROUTE_NOT_FOUND`, `*_NOT_FOUND`                                                                                      |
| 409  | `AUTH_EMAIL_EXISTS`, transition/conflict codes: `CAMPAIGN_NOT_OPEN`, `CAMPAIGN_NOT_EDITABLE`, `REGISTRATION_DUPLICATE`, `REGISTRATION_OVERLAP`, `REGISTRATION_WINDOW_CLOSED`, `TIME_SLOT_FULL`, `TIME_SLOT_INACTIVE`, `CHECK_IN_NOT_ALLOWED`, `REGISTRATION_ALREADY_CHECKED_IN`, `SCREENING_NOT_ELIGIBLE`, `DONATION_NOT_COMPLETED`, `CERTIFICATE_ALREADY_REVOKED`, `*_INVALID_TRANSITION` |
